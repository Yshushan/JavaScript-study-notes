...
```
<span style="color:red">typeof</span> type guard 有两种形式，`typeof v === typename` 和 `typeof v !== typename`，其中 `typename` 必须为 `'number'`，`'string'`，`'boolean'`，`'symbol'` 四者之一，若为其它值，TypeScript 不会将这个表达式视为 type guard。
